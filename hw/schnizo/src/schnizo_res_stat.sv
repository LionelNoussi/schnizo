...
module schnizo_res_stat import schnizo_pkg::*; #(
  ...
  assign num_expected_results = expect_result_hs ? ((issue_req_o.fu_data.fu == schnizo_pkg::ALU_LSU_LOAD) ? 2'b10 : 2'b01) : 2'b00;
  ...
  // This is necessary, since we can't assign the bound to max-1, since we might increment the counter
  // by 2 as well.
  rsrs_cnt_t assign_rsrs_next;
  rsrs_cnt_t assign_rsrs_bound;
  logic assigned_rsrs_idx_reset;

  assign assign_rsrs_bound = rsrs_cnt_t'((loop_state_i == LoopLcp1) ? NofRsrs : num_allocated_rsrs_q);
  assign assign_rsrs_next  = rsrs_cnt_t'(assigned_rsrs_idx) + rsrs_cnt_t'(num_expected_results); 
  assign assigned_rsrs_idx_reset = goto_lcp2_i || restart_i || (assign_rsrs_next == assign_rsrs_bound);
