...
def gen_experiments(hardwares, sizes, axpy_modes, dot_modes):
    ...
    for hw in hardwares:
        for mode in axpy_modes:
            ...
            for n in sizes:
                verify_script = Path(
                    f"{schnizo_dir}/sw/kernels/blas/{app}/scripts/verify.py"
                )
                experiments.extend(
                    [
                        {
...
def main():
    ...
    try:
        ...
        df["fpu_util"] = df.apply(
            lambda row: row["results"].get_metric(roi, "fpu_util"), axis=1
        )
        print(df)
        ...
        print("Failed to extract performance results.")
